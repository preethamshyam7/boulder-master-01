...
var (
	...
	oidTNAuthList = asn1.ObjectIdentifier{1, 3, 6, 1, 5, 5, 7, 1, 26}
)
...
var (
	...
	authListExtension    = signer.Extension{
               ID:       cfsslConfig.OID(oidTNAuthList),
               Critical: false,
               Value:    hex.EncodeToString(mustStapleFeatureValue),
       }
			...
			for _, ext := range extList {
				...
				case ext.Type.Equal(oidTLSFeature):
					...
					if ca.enableMustStaple {
						...
						extensions = append(extensions, authListExtension)
					}
