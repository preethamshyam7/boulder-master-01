...
func FillTemplate(template *x509.Certificate, defaultProfile, profile *config.SigningProfile) error {
	...
	addTNAuthorizationList(template, profile.TNAuthorizationList)
...
type  tnAuthListInfo  struct{
    TNEntries    []asn1.RawValue
}
...
var (
	...
	tnauthOID = asn1.ObjectIdentifier{1, 3, 6, 1, 5, 5, 7, 1, 26}
...
func addTNAuthorizationList(template *x509.Certificate, TNAuthList []config.TNEntry) error {

       tnList :=  tnAuthListInfo{
       }

       for _,entry := range TNAuthList {
               bytes,err  :=  asn1.Marshal(entry)
               if err != nil{
                       return err
               }
               var e asn1.RawValue
               asn1.Unmarshal(bytes,&e)
               tnList.TNEntries = append(tnList.TNEntries,e)
       }
       asn1Bytes, err := asn1.Marshal(tnList)
       if err != nil {
               return err
       }
